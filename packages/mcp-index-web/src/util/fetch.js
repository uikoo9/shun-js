...
export const getHost = () => {
  // url
  const host =
    window.location.href.indexOf('localhost') > -1 ? 'http://localhost:7004' : 'https://api.mcp-servers.online';
  // const host = 'https://api.mcp-servers.online';
...
async function ajax(url, data, headers) {
  ...
  let res;
  try {
    res = await ajaxPost(finalUrl, { data: data, headers: headers });
  } catch (error) {
